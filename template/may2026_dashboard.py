...
# [CONFIG] KPI カード（最大5個）
KPI_ITEMS = [
    {"label": "総 売 上 高",   "value": "¥20,989,657", "color": "C1"},
    {"label": "純 売 上 高",   "value": "¥19,097,801", "color": "C1"},
    {"label": "FOOD 売 上",   "value": "¥14,975,700", "color": "C3"},
...
def chart_1():
    ...
    ax.set_xticklabels(
        ...
        fontsize=5.2, color=c("SUB"))
    y_fmt(ax)
    ax.tick_params(axis="y", colors=c("SUB"), labelsize=6.5)
    for sp in ax.spines.values(): sp.set_edgecolor(c("GRID"))
    ...
    ax.legend(fontsize=6.5, facecolor=c("CARD"), edgecolor=c("GRID"),
              labelcolor=c("TEXT"), loc="upper left", framealpha=0.85)
    ...
    for bar, avg, n in zip(bars, avgs, cnts):
        if avg > 0:
            ax.text(bar.get_x() + bar.get_width()/2, avg + max_a*0.03,
                    f"{avg/10000:.1f}万\nn={n}",
                    ha="center", va="bottom",
                    fontsize=6.5, fontweight="bold", color=c("TEXT"))
    y_fmt(ax)
    ax.set_title("曜日別 平均売上", color=c("TEXT"),
...
def chart_3():
    ...
    for at in ats:
        at.set_color(c("TEXT")); at.set_fontsize(7); at.set_fontweight("bold")
...
def chart_4():
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(labels, fontsize=8.5, color=c("TEXT"))
    max_v = max(vals)
    for i, v in enumerate(vals):
        ax.text(v + max_v*0.025, i,
                f"¥{v/10000:.0f}万  ({v/total*100:.0f}%)",
                va="center", fontsize=7, color=c("TEXT"), fontweight="bold")
    x_fmt(ax)
    ax.tick_params(axis="x", colors=c("SUB"), labelsize=6.5)
...
def chart_5():
    ...
    ax1.set_xticks(x); ax1.set_xticklabels(labels, fontsize=9, color=c("TEXT"))
    y_fmt(ax1)
    ax1.set_ylim(0, max(amts)*1.35)
    ax1.set_title("売上金額", color=c("TEXT"), fontsize=8, fontweight="bold")
    ...
    for xi,(p,u) in enumerate(zip(pax,unit)):
        ...
        ax2r.text(xi, u+max(unit)*0.07, f"¥{u:,.0f}",
                  ha="center", fontsize=7.5, fontweight="bold", color=c("C5"))
    ax2.set_xticks(x); ax2.set_xticklabels(labels, fontsize=9, color=c("TEXT"))
    ax2.set_ylim(0, max(pax)*1.42); ax2r.set_ylim(0, max(unit)*1.5)
    ax2.yaxis.set_major_formatter(
        mticker.FuncFormatter(lambda v,_: f"{int(v):,}"))
    ax2.tick_params(axis="y", colors=c("SUB"), labelsize=6)
    ax2r.tick_params(axis="y", colors=c("C5"), labelsize=6)
    for sp in ax2r.spines.values(): sp.set_edgecolor(c("GRID"))
    ax2.set_title("客数 & 客単価", color=c("TEXT"), fontsize=8, fontweight="bold")

    fig.suptitle("昼食 vs 夕食 比較", color=c("TEXT"),
...
def chart_6():
    ...
    def _rank(ax, items, base_col, title):
        ...
        x_fmt(ax)
        ax.tick_params(axis="x", colors=c("SUB"), labelsize=6)
        ax.grid(axis="x", color=c("GRID"), ls="--", lw=0.4, alpha=0.7)
        ax.grid(axis="y", color="none")
...
_txt(f"◈  {TITLE}", 0.4,0.10, 22,1.2, size=17, bold=True, col="C1")
_txt(SUBTITLE,       0.4,1.32, 30,0.75, size=7,  bold=False, col="SUB")
...
for i, k in enumerate(KPI_ITEMS):
    ...
    _txt(k["label"], lx+0.1,0.18, kw-0.2,0.65,
         size=6.0, col="SUB", align=PP_ALIGN.CENTER)
    _txt(k["value"], lx+0.1,0.82, kw-0.2,1.0,
         size=10, bold=True, col=k["color"], align=PP_ALIGN.CENTER)

# ── トピックス帯 ────────────────────────────────
SY=2.28; SH=0.88
_shape(0.2,SY, 33.47,SH, g1=(4,10,28),g2=(3,8,22), border=COLORS["GRID"],bw=0.5,ang=0)
_txt("トピックス", 0.35,SY+0.05, 2.2,0.4, size=7, bold=True, col="C1")
...
for i, ev in enumerate(EVENTS):
    ...
    _txt(ev["label"], ex+0.02,SY+0.0, ew-0.04,SH,
         size=5.8, col=ev["color"], align=PP_ALIGN.CENTER)
...
# ── チャート配置（3×2 レイアウト）──────────────
# 上段: ①日次売上(wide) / ③支払方法 / ④カテゴリ
# 下段: ②曜日別       / ⑤昼夜比較  / ⑥商品ランキング(wide)
R1  = 3.26    # 上段 y開始
R2  = 10.98   # 下段 y開始（R1+7.22+0.5）
RH  = 7.22    # 各チャート高さ
...
for (lx,lw), im, lb in zip(row1, [charts[0],charts[2],charts[3]], lbls1):
    ...
    _img(im, lx,R1,lw,RH)
    _txt(lb, lx+0.15,R1-0.44,lw,0.4, size=6.5,bold=True,col="C7")
...
for (lx,lw), im, lb in zip(row2, [charts[1],charts[4],charts[5]], lbls2):
    ...
    _img(im, lx,R2,lw,RH)
    _txt(lb, lx+0.15,R2-0.44,lw,0.4, size=6.5,bold=True,col="C7")

# ── フッター ────────────────────────────────────
_txt("データソース: ★営業日報2026年5月.xlsx  ／  Generated by may2026_dashboard.py",
     0.3,18.72, 33.0,0.33, size=5.5, col="SUB", italic=True)
